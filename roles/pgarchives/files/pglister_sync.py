...
if __name__ == "__main__":
    ...
    for id, name in list(groups.items()):
        ...
        if not curs.fetchone()[0]:
            curs.execute("INSERT INTO listgroups (groupid, groupname, sortkey) VALUES (%(groupid)s, %(group)s, 100) RETURNING groupname", {
                'groupid': id,
                'group': name,
            })
    ...
    for l in obj:
        ...
        if curs.rowcount == 0:
            curs.execute("INSERT INTO lists (listid, listname, shortdesc, description, active, groupid, subscriber_access) SELECT %(listid)s, %(name)s, %(name)s, %(desc)s, 't', groupid, 'f' FROM listgroups WHERE groupname=%(groupname)s RETURNING listid, listname", {
                'listid': l['listid'],
                'name': l['listname'],
                'desc': l['longdesc'],
